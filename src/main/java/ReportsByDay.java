...
public class ReportsByDay implements SelectionListener {
  ...
  public ReportsByDay (SQLManager sql_man) {
    sql = sql_man;
    d = Display.getCurrent();
    window = new Shell(d, SWT.APPLICATION_MODAL | SWT.CLOSE);
    window.setText("Дневен отчет");
    try {
    ...
    try {
      ...
      if (printer.startJob(jobName)){
        ...
        if (printer.startPage()){
          ...
          gc.drawString("Отчет по дни", 200, 150);
  ...
  public void widgetSelected (SelectionEvent event) {
    ...
    if (ev == cDate) {
      try {
        ...
        for (int j=1; j<= sql.CountEqualDateItems(); j++){
          String[] col = sql.GetRowByDay(cDate.getText(), j);
    ...
    if (ev == bPrint) print("Warehouse by Day", window, table);
  }
}
